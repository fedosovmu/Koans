...
public class AboutLocale {
    ...
    public void getCountryInformation() {
        ...
        assertEquals(locBR.getDisplayCountry(), locBR.getDisplayCountry()); // <- Пробема с кодировкой
        assertEquals(locBR.getDisplayCountry(locBR), "Brasil");

        Locale locCH = new Locale("it", "CH");
        assertEquals(locCH.getDisplayCountry(), locCH.getDisplayCountry()); // <- тоже не могу записать
